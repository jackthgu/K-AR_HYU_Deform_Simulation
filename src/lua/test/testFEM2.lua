...
function ctor()
	...
	local stiffness=1E5
	local nodeIndex=20
	local nodeIndex2=100
	mSoftWorld:addAttachmentConstraint('c0', stiffness, nodeIndex, mMesh:nodePos(nodeIndex))
	mSoftWorld:addAttachmentConstraint('c1', stiffness, nodeIndex2, mMesh:nodePos(nodeIndex2)+vector3(0,5,0))

	for i=0, mMesh:getNumTetra()-1 do
		local muscleStiffness=4E5
		local fiber_direction=vector3(0,0,1)
