...
Physics_SymmetricMatrix::Physics_SymmetricMatrix( int iRows )
{
	...
	m_iRows = iRows;
	m_pData = new m_real[ m_iRows * (m_iRows+1) / 2 ];
	zero();
}
...
{
	memset( m_pData, 0, m_iRows * (m_iRows+1) / 2 * sizeof( m_real ) );
}
...
bool Physics_SymmetricMatrix::PostMultiply( Physics_LargeVector &V, Physics_LargeVector &Dst )
{
	...
	for( i=0; i<m_iRows; i++ )
	{
		for( j = i; j<m_iRows; j++ )
		{
			Dst[i].x += m_pData[i*m_iRows - i*(i-1)/2+j-i] * V[i].x;
			Dst[i].y += m_pData[i*m_iRows - i*(i-1)/2+j-i] * V[i].y;
			Dst[i].z += m_pData[i*m_iRows - i*(i-1)/2+j-i] * V[i].z;
			if( i != j )
			{
				Dst[j].x += m_pData[i*m_iRows - i*(i-1)/2+j-i] * V[i].x;
				Dst[j].y += m_pData[i*m_iRows - i*(i-1)/2+j-i] * V[i].y;
				Dst[j].z += m_pData[i*m_iRows - i*(i-1)/2+j-i] * V[i].z;
			}
		}
	}
	return true;
...
/*
void Physics_SymmetricMatrix::mult(Physics_SparseSymmetricMatrix const& a, m_real b)
{
	ASSERT(m_iRows==a.m_iRows);

	ct = m_iRows * (m_iRows+1) / 2;
	for( i=0; i<ct; i++ )
		m_pData[i] = scale * a.m_pData[i];
}

void Physics_SymmetricMatrix::operator*=(m_real b)
{
	ASSERT(m_iRows==a.m_iRows);
...
bool Physics_SymmetricMatrix::Scale( m_real scale, Physics_SymmetricMatrix &Dst )
{
...
m_real & Physics_SymmetricMatrix::operator() (int row, int col)
{
	int k;
...
void Physics_SymmetricMatrix::Copy( Physics_SymmetricMatrix &M )
{
	...
	m_iRows = M.m_iRows;
	m_pData = new m_real[m_iRows * (m_iRows+1) / 2];
	memcpy( m_pData, M.m_pData, (m_iRows * (m_iRows+1)/2) * sizeof( m_real ) );
}
...
bool Physics_SymmetricMatrix::Invert()
{
	...
	pData = new m_real[m_iRows * m_iRows];
	...
	for( i=0; i<m_iRows; i++ )
	{
		...
		pivinv=(m_real)1.0/pData[icol*m_iRows+icol];
...
void Physics_SymmetricMatrix::identity()
{
	int i;
	...
	for( i=0; i<m_iRows; i++ )
	{
		m_pData[i*m_iRows - i * (i-1) / 2] = (m_real)1.0;
	}
}
