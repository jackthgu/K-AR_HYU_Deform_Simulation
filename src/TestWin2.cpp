...
static TestWin2* g_pTestWinSingleton=NULL;
 
bool isTestWin2Initialized()
{
	return bInitialized;
}
...
// function 1
void TestWin2::updateAllFeaturePointsFromCurBarycentrics(vector3N* mFeaturePoints, vector3N* mFeaturePoints_originalShape, vector3N* mCutFeaturePoints)
{
	ASSERT(bInitialized);
#ifdef DRAW_DELTA
		...
		for (int i = 0; i < curBarycentrics.size(); i++) {
			...
			//	j=i*2;
// function 1
void TestWin::updateAllFea// function 1
void TestWin::updateAllFeaturePointsFromCurBarycentrics(vector3N* mFeaturePoints, vector3N* mFeaturePoints_originalShape, vector3N* mCutFeaturePoints)
{
	ASSERT(bInitialized);
		...
		//MOutputToFile("points.txt", ("{ "));
		for (int i = 0; i < curBarycentrics.size(); i++) {
			space::Point aturePointsFromCurBarycentrics(vector3N* mFeaturePoints, vector3N* mFeaturePoints_originalShape, vector3N* mCutFeaturePoints)
{
	ASSERT(bInitialized);
#ifdef DRAW_DELTA
		BillboardLineList *line = new BillboardLineList("track", curBarycentrics.size(), 1);
...
int TestWin2::fIdxToFeatureIndex(int fIdx) { int nv=mFeaturePoints->size(); return _fidxToFeatureIndex(fIdx+nv);}

void TestWin2::nativePreRender() {
...
int TestWin2::handleRendererEvent(int ev) {
	static int pushed_x;
	static int pushed_y;
...
// function 4
// FrameMoveObject
int TestWin2::FrameMove(float fElapsedTime) {
	//timer.start();
...
TestWin2::TestWin2(int x, int y, int w, int h, MotionPanel &mp, FltkRenderer &renderer) :
		FlLayout(x, y, w, h),
		m_motionPanel(mp),
		mRenderer(renderer),
		mFeaturePoints(NULL),
...
OBJloader::Mesh *tetraToObjfortestwin2(TetraMesh &srctet, const char *facefile, int surface_vtx_num)
{
	std::ifstream fin;
	CTextFile file;
...
void TestWin2::LoadDeformedMesh(){
...
void TestWin2::onCallback(FlLayout::Widget const &w, Fl_Widget *pWidget, int userData) {
    if(w.mId=="draw MR")
	{
...
OBJloader::Mesh *TestWin2::UpdateMesh(OBJloader::Mesh *source) {
...
void TestWin2::setLinearConstraint() {
	for (int i = 0; i < mMesh.getNumTetra(); i++) {
...
void TestWin2::loadTetMesh(const char *mesh) {
	bool bInit = true;
...
void TestWin2::tetToObj()
{
	//std::cout << "Tet to obj" << std::endl;
	//mMeshDeformer = createHarmonicMeshDeformer();
	mMeshDeformer = createMeanValueMeshDeformer();
	tet_to_obj = tetraToObjfortestwin2(mMesh, _meshName + ".face", getNumSurfaceVertex());  //"../mesh/iamgroot101_surface.1.rearranged.face"
...
void TestWin2::NanCheck(OBJloader::Mesh *target) {
    int count = 0;
...
void TestWin2::ChangeError(double pcm, double rms) {
	char pcmString[32];
	char rmsString[32];
...
// function 2
double* estimateSurfaceVerticesFromCurBarycentrics_tw2()
{
	// without soft-body simulation
	vector3N featurepoints;
	TestWin2* t=g_pTestWinSingleton;
	featurepoints=*g_pTestWinSingleton->mFeaturePoints;
//	TestWin2::updateAllFeaturePointsFromCurBarycentrics(&featurepoints, t->mFeaturePoints_originalShape);
