...
{
public:
	vertex()					: m_Pnt(0.0F, 0.0F),m_index(-1)			{}
	vertex(const vertex& v)		: m_Pnt(v.m_Pnt),m_index(v.m_index)		{}
	vertex(const PointF& pnt)	: m_Pnt(pnt),m_index(-1)				{}
	vertex(REAL x, REAL y)		: m_Pnt(x, y),m_index(-1)				{}
	vertex(int x, int y)		: m_Pnt((REAL) x, (REAL) y),m_index(-1)	{}
	vertex(REAL x, REAL y, int index)		: m_Pnt(x, y),m_index(index){}
	...
	int GetIndex() const { ASSERT(m_index!=-1); return m_index;}
...
//		if (y < (m_Center.Y - m_R)) return false;
	...
	triangle& operator=(const triangle& tri) const
	{
		...
		self.m_Vertices[0]=tri.m_Vertices[0];
		self.m_Vertices[1]=tri.m_Vertices[1];
		self.m_Vertices[2]=tri.m_Vertices[2];
		self.m_Center=tri.m_Center;
		self.m_R=tri.m_R;
		self.m_R2=tri.m_R2;
		return self;
	}
