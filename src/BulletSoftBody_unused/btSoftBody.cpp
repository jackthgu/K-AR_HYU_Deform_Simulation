...
extern bool			CheckContactTaesoo(	btSoftBody* psb,
									btRigidBody* prb,
									const btVector3& x,
									btScalar margin,
									btSoftBody::sCti& cti);
									...
									btSoftBody::sCti& cti)
{
	...
	return CheckContactTaesoo(psb, prb, x, margin, cti);
	/*
	btVector3			nrm;
	btCollisionShape*	shp=prb->getCollisionShape();
	//btAssert(shp->isConvex()); taesoo
...
//static void			UpdateConstantsTaesoo(btSoftBody* psb);
//
