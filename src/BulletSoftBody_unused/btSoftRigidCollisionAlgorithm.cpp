...
void btSoftRigidCollisionAlgorithm::processCollision (btCollisionObject* body0,btCollisionObject* body1,const btDispatcherInfo& dispatchInfo,btManifoldResult* resultOut)
{
	...
	btCollisionObject* softBody =  m_isSwapped? body1 : body0;
	btCollisionObject* rigidCollisionObject = m_isSwapped? body0 : body1;
	
	switch(softBody->getInternalType())
	{
	case btCollisionObject::CO_SOFT_BODY:
			((btSoftBody*)softBody)->defaultCollisionHandler(rigidCollisionObject);
			break;
	case btCollisionObject::CO_SOFT_BODY2:
			((btSoftBody2*)softBody)->defaultCollisionHandler(rigidCollisionObject);
			break;
		default:
